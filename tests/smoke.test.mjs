...
test('home content and metadata are in the initial HTML, without JavaScript', async () => {
  ...
  assert.match(
    ...
    'LLM-readable content must be discoverable in server-rendered HTML',
  )
  assert.doesNotMatch(
    html,
...
test('generated LLM files are served as plain text with expanded page content', async () => {
  const [index, full] = await Promise.all(
    ['/llms.txt', '/llms-full.txt'].map(async (path) => {
      ...
      assert.equal(
        response.status,
        200,
        `${path} must be generated by the build`,
      )
      assert.match(response.headers.get('content-type'), /^text\/plain(?:;|$)/)
      const text = await response.text()
      assert.match(text, /^# Relay\r?\n/)
      return text
    }),
  )
  assert.match(
    index,
    /\[Read the full public content\]\([^)]*\/llms-full\.txt\)/,
  )
  assert.match(
    full,
    /## Add an update\r?\n\r?\n1\. Choose a project: Website or Mobile app\./,
    'The full file must contain the demo source content, not just its index entry',
  )
})
